      ...
      <el-form ref="condition" :model="condition" class="flex padding-top margin-right" label-position="left">
        ...
        <el-form-item label="添加时间" label-width="80px">
          <el-date-picker v-model="condition.time" type="daterange" value-format="yyyy-MM-dd" range-separator="至"
            start-placeholder="开始日期" end-placeholder="结束日期" size="small" @change="changeTime()" style="width: 250px">
          </el-date-picker>
        </el-form-item>
      </el-form>
          ...
          <div>
            <div class="flex">
              ...
              <div class="margin-left">
                <input type="file" ref="file" style="display: none" v-on:change="uploadFile($event)" />
            ...
            <div style="margin-left: 10%">
              <img v-if="form.imgUrl" :src="form.imgUrl" alt="" style="width: 100px; height: 100px" />
            </div>
          </div>
        </el-form>
      </div>
...
export default {
  ...
  methods: {
    // 查询首页数据
    queryData() {
      this.get("/goodsManagement/queryChangeCategory", {
    ...
    deleteItem(idList) {
      this.$confirm("此操作将永久删除文件, 是否继续?", "提示", {
        ...
        .then(() => {
          this.post("/goodsManagement/deleteChangeCategory", {
            idList: idList,
          }).then((res) => {
            this.queryData();
            this.$message({
      ...
      if (this.buttonType === 2) {
        this.$refs.forms.validate((valid) => {
          if (valid) {
            ...
            axios({
              method: "POST",
              url: "/api/goodsManagement/addChangeCategory",
              headers: {
        ...
        this.$refs.forms.validate((valid) => {
          if (valid) {
            ...
            axios({
              method: "POST",
              url: "/api/goodsManagement/updateChangeCategory",
              headers: {
                "Content-Type": "multipart/form-data",
              },
              data: data,
